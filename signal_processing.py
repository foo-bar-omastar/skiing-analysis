...
def generate_stroke_dataframe(filepath, i):
    ...
    df["Time left (sec)"] = np.linspace(
        np.min(df["Time left (sec)"]), np.max(df["Time left (sec)"]), len(df.index)
    )
    # Same for min
    df["Time left (min)"] = np.linspace(
    ...
    df["Time right (sec)"] = np.linspace(
        np.min(df["Time right (sec)"]), np.max(df["Time right (sec)"]), len(df.index)
    )
    # Same for min
...
def ImportData(filepath, skier):
    ...
    df["Time left (sec)"] = df["Time left (min)"] * 60
    # Adding timeseries in seconds
    df["Time right (sec)"] = df["Time right (min)"] * 60
...
def GetStrokes(
    ...
    for i in range(len(peaks)):
        ...
        force_area[i] = np.trapz(
            ...
            df["Time " + pole.lower() + " (sec)"][start:stop].values,
        )
    ...
    df_peaks = pd.DataFrame.from_dict(data, orient='index')
    df_peaks.transpose()
    
    # functional
    #clean_dict = filter(lambda k: not isnan(data[k]), data)
    
    from math import isnan
    
    clean_dict = {k: v for k, v in data.items() if not isnan(v)}
    
    d = clean_dict
    df_peaks = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in d.items() ]))
    '''
